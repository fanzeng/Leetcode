class Solution(object):
    def maxProfit(self, prices):
        ...
        """
        self.d = {}
        return self.maxProfitDP(prices, 0, None)
    # status:
    # 1: is holding, can hold or sell
    # 0: neither holding nor cooling, can buy
    # -1: just sold, must cool
    def maxProfitDP(self, prices, status, bought_price):
        if len(prices) == 0:
            return 0
        h = self.getHash(prices, status, bought_price)
        if self.d.get(h) is not None:
            return self.d[h]
        if status == 1:
            profit_if_sell = prices[0] - bought_price + self.maxProfitDP(prices[1:], -1, None)
            profit_if_hold = self.maxProfitDP(prices[1:], 1, bought_price)
            res = max(profit_if_sell, profit_if_hold)
            self.d[h] = res
            return res
        if status == 0:
            profit_if_buy = self.maxProfitDP(prices[1:], 1, prices[0])
            profit_if_hold = self.maxProfitDP(prices[1:], 0, None)
            res = max(profit_if_buy, profit_if_hold)
            self.d[h] = res
            return res
        if status == -1:
            res = self.maxProfitDP(prices[1:], 0, None)
            self.d[h] = res
            return res

    def getHash(self, prices, status, bought_price):
        h = ''
        for price in prices:
            h += str(price) + ','
        h += ':' + str(status)
...
test = Solution()
print test.maxProfit([1,2,3,0,2]) # 3
print test.maxProfit([48,12,60,93,97,42,25,64,17,56,85,93,9,48,52,42,58,85,81,84,69,36,1,54,23,15,72,15,11,94])

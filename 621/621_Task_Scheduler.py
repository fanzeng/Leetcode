class Solution(object):
    def leastInterval(self, tasks, n):
        ...
        d_task_cool = {}
        for t in tasks:
            if t in d_task_count:
                d_task_count[t] += 1
            else:
                d_task_count[t] = 1
                d_task_cool[t] = 0
        return self.leastIntervalDP(d_task_count, d_task_cool, 0)

    def leastIntervalDP(self, d_task_count, d_task_cool, time):
        # print d_task_count, d_task_cool, time
        if len(d_task_count.keys()) == 0:
            return time
        h = self.getHash(d_task_count, d_task_cool)
        d_res = self.d.get(h)
        if d_res is not None:
            return time + d_res
        min_cool = min(d_task_cool.values())
        delta_time = min_cool
        for task in d_task_cool.keys():
            d_task_cool[task] -= min_cool

        l_total_time = []
        for task in d_task_count.keys():
            if d_task_cool.get(task) == 0:
                d_task_count_new = {}
                d_task_cool_new = {}
                for t in d_task_count.keys():
                    if t != task:
                        d_task_count_new[t] = d_task_count[t]
                        d_task_cool_new[t] = d_task_cool[t] - 1 if d_task_cool[t] >= 1 else 0
                    else:
                        new_task_count = d_task_count[t]-1
                        if new_task_count > 0:
                            d_task_count_new[t] = new_task_count
                            d_task_cool_new[t] = self.n
                time_if_do_task = self.leastIntervalDP(d_task_count_new, d_task_cool_new, time+delta_time+1)
                l_total_time.append(time_if_do_task)
        min_total_time = min(l_total_time)
        self.d[h] = min_total_time - time
        return min_total_time


    def getHash(self, d_task_count, d_task_cool):
        h = ''
        keys_sorted_by_count = [k[0] for k in sorted(d_task_count.items(), key=lambda x : x[1])]
        for task in keys_sorted_by_count:
            h += str(d_task_count[task]) + '_' + str(d_task_cool[task])
        return h
